...
<head>
  ...
  <meta name="description" content="Guided meditations, breathing exercises, sleep stories and mindfulness tools to reduce stress and improve focus." />
  <style>
    :root{
      --bg: #f3f7fb;
      --card: #ffffff;
    ...
    *{box-sizing:border-box}
    ...
    .btn{padding:10px 14px;border-radius:12px;border:none;font-weight:700;cursor:pointer;text-decoration:none}
    .btn-ghost{background:transparent;border:1px solid rgba(15,23,42,0.06);color:inherit}
    .btn-primary{background:var(--accent);color:#fff;box-shadow:0 10px 30px rgba(108,99,255,0.18)}

    /* Hero */
    .hero{display:grid;grid-template-columns:1fr 420px;gap:28px;align-items:center;margin-bottom:36px}
    .hero h1{font-size:36px;margin:0;line-height:1.03}
    .hero p{margin:14px 0 0;color:var(--muted);max-width:55ch}
    .hero-actions{margin-top:18px;display:flex;gap:12px;align-items:center}
    .statbar{display:flex;gap:12px;margin-top:18px;color:var(--muted);font-size:13px}

    /* Card / preview */
    .card{background:var(--card);border-radius:16px;padding:16px;box-shadow:var(--shadow);border:1px solid rgba(15,23,42,0.04)}
    .player{display:flex;align-items:center;gap:14px}
    .play{
      width:56px;height:56px;border-radius:12px;background:linear-gradient(135deg,var(--accent),#98a7ff);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:20px;box-shadow:0 8px 20px rgba(108,99,255,0.18)
    }
    .track{flex:1}
    .track h4{margin:0;font-size:16px}
    .track p{margin:6px 0 0;color:var(--muted);font-size:13px}

    /* Sections */
    .grid-3{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;margin-top:18px}
    .feature{padding:18px;border-radius:12px;background:linear-gradient(180deg,#fff, #fbfdff);border:1px solid rgba(15,23,42,0.04)}
    .feature h3{margin:0 0 8px 0}
    .feature p{margin:0;color:var(--muted);font-size:14px}

    /* Sessions list */
    .sessions{display:grid;grid-template-columns:repeat(2,1fr);gap:12px}
    .session{display:flex;gap:12px;align-items:center;padding:12px;border-radius:12px;background:var(--card);border:1px solid rgba(15,23,42,0.04)}
    .session .thumb{width:64px;height:64px;border-radius:10px;flex-shrink:0;background:linear-gradient(135deg,var(--accent-2),#8ae3d0);display:flex;align-items:center;justify-content:center;color:white;font-weight:700}

    /* Testimonials */
    .testimonials{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:12px}
    .testimonial{padding:14px;border-radius:12px;background:linear-gradient(180deg,#fff,#f8fbff);border:1px solid rgba(15,23,42,0.04)}
    .testimonial p{margin:0;color:var(--muted)}
    .author{margin-top:12px;font-weight:700;font-size:14px}

    /* CTA */
    .cta{
      margin-top:26px;
      background:linear-gradient(90deg,var(--accent) 0%, #4fd1c5 100%);
      color:white;padding:22px;border-radius:14px;display:flex;justify-content:space-between;align-items:center;box-shadow:0 16px 40px rgba(79,70,229,0.12)
    }
    .cta .left{max-width:70%}
    .cta h3{margin:0;font-size:18px}
    .cta p{margin:6px 0 0;color:rgba(255,255,255,0.9)}

    footer{display:flex;justify-content:space-between;align-items:center;padding-top:20px;color:var(--muted);font-size:13px;margin-top:26px;border-top:1px solid rgba(15,23,42,0.04)}

    /* Utilities & responsive */
    .muted{color:var(--muted)}
    .pill{display:inline-block;padding:6px 10px;border-radius:999px;background:rgba(108,99,255,0.08);color:var(--accent);font-weight:700;font-size:13px}
  ...
  <div class="wrap" role="document">
    <header>
      ...
      <nav aria-label="Main navigation">
        ...
        <a class="btn btn-primary" href="#start">Start meditating</a>
      </nav>
    </header>
    ...
    <section class="hero" aria-labelledby="hero-title">
      <div>
        ...
        <p>Guided meditations, breathing exercises and bedtime stories to help lower stress, sleep better and boost focus — in as little as 5 minutes a day.</p>
        ...
        <div class="statbar" aria-hidden="true">
          ...
          <div style="border-left:1px solid rgba(15,23,42,0.06);padding-left:12px"><strong>4.8</strong> average rating</div>
        </div>
      </div>
      ...
      <aside class="card" aria-label="Featured session preview">
        ...
        <div style="margin-top:14px;display:flex;gap:8px;">
          <button class="btn btn-ghost" onclick="alert('Play sample — replace with audio player')">Play sample</button>
          <button class="btn" style="background:#eefbf6;color:var(--accent-2);border-radius:10px" onclick="location.href='#start'">Start session</button>
        </div>
      </aside>
    </section>
    ...
    <section id="sessions" style="margin-top:22px" aria-labelledby="sessions-title">
      ...
      <div class="sessions">
        <div class="session card" role="article" aria-label="Morning breathing">
          ...
          <div>
            <div style="font-weight:800">Morning breathing</div>
            <div class="muted" style="margin-top:6px;font-size:13px">2–6 minutes · Energy & focus</div>
          </div>
          <div style="margin-left:auto;color:var(--muted)">4:00</div>
        </div>

        <div class="session card" role="article" aria-label="Stress release">
          <div class="thumb" style="background:linear-gradient(135deg,#6c63ff,#9bb0ff)">SR</div>
          <div>
            <div style="font-weight:800">Stress release</div>
            <div class="muted" style="margin-top:6px;font-size:13px">Guided letting go · 10 minutes</div>
          </div>
          <div style="margin-left:auto;color:var(--muted)">10:00</div>
        </div>

        <div class="session card" role="article" aria-label="Sleep story">
          <div class="thumb" style="background:linear-gradient(135deg,#00bfa6,#3bd7c9)">SS</div>
          <div>
            <div style="font-weight:800">Sleep story — Moonlit sea</div>
            <div class="muted" style="margin-top:6px;font-size:13px">Calming narration · 22 minutes</div>
          </div>
          <div style="margin-left:auto;color:var(--muted)">22:00</div>
        </div>

        <div class="session card" role="article" aria-label="Focus practice">
          <div class="thumb" style="background:linear-gradient(135deg,#ffd166,#ff8a66)">FP</div>
          <div>
            <div style="font-weight:800">Focus practice</div>
            <div class="muted" style="margin-top:6px;font-size:13px">Concentration meditation · 12 minutes</div>
          </div>
          <div style="margin-left:auto;color:var(--muted)">12:00</div>
        </div>
      </div>
    </section>
